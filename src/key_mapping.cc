...
const std::map<uint64_t, uint64_t> xkb_to_physical_key_map = {
...
const std::map<uint64_t, uint64_t> gtk_keyval_to_logical_key_map = {
...
extern "C" uint64_t apply_id_plane(uint64_t logical_id, uint64_t plane) {
  return (logical_id & kValueMask) | plane;
}

extern "C" uint64_t event_to_physical_key(struct wlr_event_keyboard_key *event) {
...
extern "C" uint64_t event_to_logical_key(struct wlr_event_keyboard_key *event, struct xkb_state *state) {
...
extern "C" uint32_t event_to_unicode(struct wlr_event_keyboard_key *event, struct xkb_state *state) {
  uint32_t keycode = event->keycode + 8;
  xkb_keysym_t keyval = xkb_state_key_get_one_sym(state, keycode);
...
extern "C" uint64_t event_to_timestamp(struct wlr_event_keyboard_key *event) {
  return kMicrosecondsPerMillisecond * static_cast<double>(event->time_msec);
}