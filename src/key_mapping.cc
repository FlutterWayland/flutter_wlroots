...
extern "C" uint64_t xkb_to_physical_key(uint64_t xkb_key) {
    auto it = xkb_to_physical_key_map.find(xkb_key);
    return it->second;
}


extern "C" uint64_t gtk_key_to_logical_key(uint64_t xkb_key){
    auto it = gtk_keyval_to_logical_key_map.find(xkb_key);
    return it->second;
}
