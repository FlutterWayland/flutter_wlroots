...
static inline int _write_##suffix(uint8_t **pbuffer, value_type value, size_t *remaining) { \
	return _write(pbuffer, &value, sizeof value, remaining); \
} \
static inline int _read_##suffix(uint8_t **pbuffer, value_type* value, size_t *remaining) { \
